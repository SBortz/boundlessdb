...
export class EventStore {
    ...
    async read<E extends Event = Event>(query: Query): Promise<QueryResult<E>> {
        ...
        return new QueryResult<E>(events as StoredEvent<E>[], position, query.conditions);
    }
    ...
    private checkAndReindexIfNeeded(): void {
        ...
        if (
            typeof storage.getConfigHash !== 'function' ||
            typeof storage.setConfigHash !== 'function'
        ) {
            return;
        }
    ...
    private buildConditionsFromEvents<E extends Event>(
        ...
        for (const event of events) {
            ...
            for (const extracted of extractedKeys) {
                const cond: ConstrainedCondition = {
                    type: event.type,
                    key: extracted.name,
