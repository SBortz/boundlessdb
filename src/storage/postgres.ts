            ...
            try {
                ...
                const result = await client.query<{ position: string }>(
                    `INSERT INTO events (event_id, event_type, data, metadata, timestamp)
                     VALUES ${eventValues.join(', ')} RETURNING position`,
                    eventParams
                );
    ...
    async setConfigHash(hash: string): Promise<void> {
        ...
        await this.pool.query(
            `INSERT INTO metadata (key, value)
             VALUES ('config_hash', $1) ON CONFLICT (key) DO
            UPDATE SET value = $1`,
            [hash]
        );
    }
    ...
    async reindexBatch(
        ...
        while (true) {
            ...
            try {
                ...
                await client.query('DELETE FROM event_keys WHERE position >= $1 AND position <= $2', [
                    minPos.toString(),
                    maxPos.toString(),
                ]);
                ...
                await client.query(
                    `INSERT INTO metadata (key, value)
                     VALUES ('reindex_position', $1) ON CONFLICT (key) DO
                    UPDATE SET value = $1`,
                    [maxPos.toString()]
                );
    ...
    private buildPostgresQuery(
        ...
        if (conditions.length === 0) {
            ...
            if (fromPosition !== undefined) {
                sql += backwards ? ` WHERE position < $${paramIndex}` : ` WHERE position > $${paramIndex}`;
            ...
            multiTypeConstrained.forEach((c, i) => {
                ...
                if (isMultiKey) {
                    ...
                    let cteSql = `
                        ...
                        WHERE k.key_name = ${namePh}
                          AND k.key_value = ${valPh}
                ...
                if (isMultiKey) {
                    const intersectParts = c.keys.map(key => {
                        ...
                        let part = `
                            SELECT position
                            FROM event_keys
                            WHERE key_name = ${keyNameParam}
                              AND key_value = ${keyValueParam}`;
                        if (positionFilter !== null) {
                            part += ` AND position > $${paramIndex}`;
                            params.push(positionFilter);
                            paramIndex++;
                    ...
                    let cteSql = `
                        ...
                        FROM event_keys k
                                 INNER JOIN events e ON e.position = k.position
                        WHERE k.key_name = ${keyNameParam}
                          AND k.key_value = ${keyValueParam}`;
                    if (positionFilter !== null) {
                        cteSql += ` AND e.position > $${paramIndex}`;
                        params.push(positionFilter);
                        paramIndex++;
